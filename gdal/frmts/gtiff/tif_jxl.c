...
typedef struct {
        ...
        uint32_t        worker_threads;         /* number of worker threads. default to 0 */
...
JXLPreDecode(TIFF* tif, uint16_t s)
{
        ...
        if( sp->decoder == NULL )
        {
            ...
            if( sp->worker_threads > 1 )
            {
                if( sp->thread_paraller_runner == NULL )
                {
                    sp->thread_paraller_runner = JxlThreadParallelRunnerCreate(
                        NULL, sp->worker_threads);
                }
                if( sp->thread_paraller_runner != NULL )
                {
                    JxlDecoderSetParallelRunner(sp->decoder,
                                                JxlThreadParallelRunner,
                                                sp->thread_paraller_runner);
                }
            }
        }
        else
...
JXLCleanup(TIFF* tif)
{
        ...
        if( sp->thread_paraller_runner )
            JxlThreadParallelRunnerDestroy(sp->thread_paraller_runner);
...
static const TIFFField JXLFields[] = {
        ...
        { TIFFTAG_JXL_WORKER_THREADS, 0, 0, TIFF_ANY, 0, TIFF_SETGET_UINT32,
          TIFF_SETGET_UNDEFINED,
          FIELD_PSEUDO, FALSE, FALSE, "WorkerThreads", NULL }
};
...
JXLVSetField(TIFF* tif, uint32_t tag, va_list ap)
{
	static const char module[] = "JXLVSetField";
        ...
        switch (tag) {
            ...
            case TIFFTAG_JXL_WORKER_THREADS:
            {
                sp->worker_threads = va_arg(ap, uint32_t);
                return 1;
            }
        ...
        switch (tag) {
            ...
            case TIFFTAG_JXL_DISTANCE:
                *va_arg(ap, float*) = sp->distance;
                break;
            case TIFFTAG_JXL_WORKER_THREADS:
                *va_arg(ap, uint32_t*) = sp->worker_threads;
                break;
...
int TIFFInitJXL(TIFF* tif, int scheme)
{
        ...
        */
        tif->tif_data = (uint8*) _TIFFcalloc(1, sizeof(JXLState));
        ...
        */
        ...
        sp->decoder = NULL;
        sp->thread_paraller_runner = NULL;
